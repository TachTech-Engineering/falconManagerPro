...
const FalconDashboard = () => {
  ...
  const [credentials, setCredentials] = useState({ clientId: '', clientSecret: '', baseUrl: 'https://api.crowdstrike.com' });
  ...
  }, [isAuthenticated, fetchAllData, fetchDetections, fetchHosts]);
  ...
  const handleBulkUpdate = async (status) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchDetections();
        showNotification(`${selectedDetections.length} detections updated successfully`);
      } else {
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={() => handleGenerateReport('detections')}
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${getSeverityColor(detection.severity)}`}>
                              {detection.severity}
                            </span>
                            <span className="text-sm text-gray-600 capitalize">{detection.status}</span>
                            <span className="text-sm text-gray-500">Assigned: {detection.assigned_to}</span>
                          </div>
                          <h3 className="text-lg font-semibold text-gray-800 mb-1">{detection.name}</h3>
                          <div className="flex items-center space-x-4 text-sm text-gray-600">
                            <span>Host: <span className="font-medium">{detection.host}</span></span>
                            <span>Behavior: {detection.behavior}</span>
                ...
                {iocs.map((ioc) => (
                  <div key={ioc.id} className="border rounded-lg p-4">
                    <div className="flex justify-between">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs ${getSeverityColor(ioc.severity)}`}>
                        {ioc.severity}
                      </span>
                    </div>
                  </div>
                ...
                {playbooks.map((playbook) => (
                  <div key={playbook.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      ...
                      <button
                        onClick={() => handleExecutePlaybook(playbook.id, detections[0]?.id)}
                        className="flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700"
                      >
      ...
      {showViewDialog && (
        <ViewDialog
          ...
          currentFilters={{ severity: selectedSeverity, search: searchQuery }}
        />
      )}

      {/* Close by Hash Dialog */}
      {showHashDialog && (
        <CloseByHashDialog
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        ...
        <p className="text-sm text-gray-600 mb-4">
          This will save your current filters and layout settings
        </p>
        <div className="flex justify-end space-x-2">
          <button onClick={onClose} className="px-4 py-2 bg-gray-200 rounded-lg">
            Cancel
          ...
          <button
            onClick={() => onSave({ name: viewName, filters: currentFilters })}
...
// Close by Hash Dialog Component
...
// Hash Analysis Dialog Component
...
// IOC Exclusion Dialog Component
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        ...
        <p className="text-sm text-gray-600 mb-4">
          This will prevent future detections for this hash
        </p>
        <div className="space-y-4">
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 border rounded-lg"
              rows="3"
              placeholder="Why is this being excluded? (e.g., Internal tool, approved software)"
            />
          </div>
          <label className="flex items-center">
            <input
